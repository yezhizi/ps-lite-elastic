...
void EmptyHandler(const KVMeta &req_meta, const KVPairs<Val> &req_data,
                  KVServer<Val> *server) {
  if (req_meta.push) {
    KVPairs<float> res;
...
void RunWorker() {
  ...
  LL << "num = " << num
     << ", Push average time cost: " << (end - start).count() / 1e6 << "ms";
  ...
  LL << "num = " << num
     << ", Pull average time cost: " << (end - start).count() / 1e6 << "ms";
}
